...
class ActionSpecial(action_types.ActionTemplate):
	...
	def start(self):
        ...
        ##MODIFIED
        # pattern: hands on knees SIMILAR TO ACTION.APPROACH.PY FILE
        if self.tag_id == ID_HANDS_ON_KNEES
        ...
        # pattern: pointing down
        if self.tag_id == ID_POINTING_DOWN
            steps = 100

        # pattern: moving arm in circle
        if self.tag_id == ID_MOVING_ARM_IN_CIRCLE
            steps = 100

        # pattern: waving
        if self.tag_id == ID_WAVING
            steps = 100

        # pattern: hands up
        if self.tag_id == ID_HANDS_UP
            steps = 100
	...
	def service(self):
        ...
        # pattern: hands on knees similar to action_approach.py
        if self.tag_id == ID_HANDS_ON_KNEES
            # read clock
